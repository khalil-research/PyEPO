...
This page shows PyTorch training-loop templates for the methods in ``pyepo.func``. See :doc:`function` for method inputs and options.
...
Each recipe below shows the method-specific part of the training loop.
...
RFWO returns a regularized solution. The example below uses solution-level MSE against :math:`\mathbf{w}^*(\mathbf{c})`.
...
Black-box methods return a predicted solution. The example below uses an objective-value loss, :math:`|\langle \mathbf{c}, \hat{\mathbf{w}} \rangle - \mathbf{c}^\top \mathbf{w}^*(\mathbf{c})|`.
...
Contrastive methods train against a cached pool of solutions. ``solve_ratio`` controls how often new instances are solved exactly during training, and ``dataset`` seeds the pool with optimal solutions. See :doc:`../advanced/pool` for details on the solution-pool mechanism.

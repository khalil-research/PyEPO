...
def test_assignment_ir():
    ...
    c = dsl.Parameter((3, 3))
    prob = dsl.Problem(dsl.Minimize(dsl.sum(c * x)), [x.sum(axis=1) == 1, x.sum(axis=0) == 1])
...
def _kw(backend):
    ...
    return {"pyomo": {"solver": _PYOMO_SOLVER}, "ortools": {"solver": _ORTOOLS_SOLVER}}.get(
        backend, {}
    )
...
def test_compiled_model_rebuild_preserves_problem_and_backend_config(backend):
    x = dsl.Variable(3, vtype=EPO.BINARY)
    c = dsl.Parameter(3)
    comp = dsl.Problem(dsl.Maximize(c @ x), [x.sum() <= 2]).compile(backend=backend, **_kw(backend))
...
def test_compiled_copy_preserves_objective(backend):
    x = dsl.Variable(3, vtype=EPO.BINARY)
    c = dsl.Parameter(3)
    comp = dsl.Problem(dsl.Maximize(c @ x), [x.sum() <= 1]).compile(backend=backend, **_kw(backend))
...
def test_relax_matches_legacy_lp(backend):
    ...
    c = dsl.Parameter(5)
    comp_lp = (
        dsl.Problem(dsl.Maximize(c @ x), [W @ x <= cap])
        .compile(backend=backend, **_kw(backend))
        .relax()
    )
...
def test_portfolio_quadratic_constraint_solves(backend):
    ...
    x = dsl.Variable(5, lb=0)
    c = dsl.Parameter(5)
    comp = dsl.Problem(dsl.Maximize(c @ x), [x.sum() == 1, x @ cov @ x <= budget]).compile(
        backend=backend, **_kw(backend)
    )
...
def test_assignment_solves_to_permutation(backend):
    ...
    c = dsl.Parameter((n, n))
    comp = dsl.Problem(
        dsl.Minimize(dsl.sum(c * x)), [x.sum(axis=1) == 1, x.sum(axis=0) == 1]
    ).compile(backend=backend, **_kw(backend))
...
def test_mixed_vtype_solves(backend):
    ...
    c = dsl.Parameter(3)
    comp = dsl.Problem(dsl.Maximize(c @ x), [x.sum() <= 4.5]).compile(
        backend=backend, **_kw(backend)
    )
...
def test_setfullobj_matches_scatter():
    ...
    c = dsl.Parameter(2)
    comp = dsl.Problem(dsl.Minimize(c @ x + np.array([5.0]) @ y), [x.sum() + y.sum() >= 1]).compile(
        "gurobi"
    )
...
def test_partial_prediction_solves(backend):
    ...
    c = dsl.Parameter(2)
    d = np.array([5.0, 5.0])
    comp = dsl.Problem(dsl.Minimize(c @ x + d @ y), [x[0] + y[0] >= 1, x[1] + y[1] >= 1]).compile(
        backend=backend, **_kw(backend)
    )
...
def test_full_prediction_fixed_cost_dataset_includes_offset(backend):
    ...
    comp = dsl.Problem(dsl.Minimize((c + d) @ x), [x.sum() >= 1]).compile(
        backend=backend, **_kw(backend)
    )
...
def test_aux_variable_solves(backend):
    ...
    c = dsl.Parameter(2)
    comp = dsl.Problem(dsl.Maximize(c @ x), [x.sum() - y <= 0]).compile(
        backend=backend, **_kw(backend)
    )
    comp.setObj([1.0, 1.0])
...
def test_constraints_named(backend):
    x = dsl.Variable(2, vtype=EPO.BINARY)
    c = dsl.Parameter(2)
    comp = dsl.Problem(dsl.Maximize(c @ x), [np.ones((1, 2)) @ x <= 1]).compile(
        backend=backend, **_kw(backend)
    )
    if backend == "gurobi":  # COPT needs no explicit model update
        comp._model.update()
    names = [
        con.ConstrName if backend == "gurobi" else con.name for con in comp._model.getConstrs()
    ]
